...
// Mapping for standard tailwind text classes
...
// First replace standard classes with explicit pixel values increased by 40%
for (const [cls, size] of Object.entries(standardClasses)) {
  const newSize = Math.round(size * 1.4);
  // Using negative lookbehind/lookahead to ensure it's a standalone class
  // Since js doesn't support lookbehind in all versions seamlessly, we use boundaries
...
// Then replace arbitrary text-[*px] classes
